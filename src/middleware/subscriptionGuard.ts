...
export async function checkSubscription(schoolId: string): Promise<SubscriptionCheckResult> {
  try {
    ...
    if (school.status === 'ACTIVE') {
      if (school.subscriptionExpiresAt && now >= new Date(school.subscriptionExpiresAt)) {
        return { isActive: false, reason: `Subscription expired on ${school.subscriptionExpiresAt}`, school };
      }
      return { isActive: true, school };
    }

    if (school.status === 'TRIAL') {
      if (school.trialEndsAt && now < new Date(school.trialEndsAt)) {
        return { isActive: true, school };
      }
