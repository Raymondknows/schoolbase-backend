...
export class PdfReportGenerator {
  ...
  static async generateReportCard(input: ReportCardInput): Promise<Uint8Array> {
    ...
    await this.drawFooter(pdfDoc, page, input);
  ...
  private static async drawFooter(pdfDoc: PDFDocument, page: PDFPage, input: ReportCardInput) {
    const { width, height } = page.getSize();
    ...
    // If a signature image URL is present, attempt to embed it for the Principal column
    if (input.signatureUrl) {
      try {
        let imageBytes: Uint8Array | null = null;
        if (input.signatureUrl.startsWith('/uploads/')) {
          const imagePath = path.join(process.cwd(), input.signatureUrl);
          try {
            imageBytes = await fs.readFile(imagePath);
          } catch (e) {
            imageBytes = null;
          }
        } else if (input.signatureUrl.startsWith('http')) {
          try {
            const resp = await fetch(input.signatureUrl);
            if (resp.ok) {
        ...
        if (imageBytes) {
          ...
          const lower = input.signatureUrl.toLowerCase();
          if (lower.endsWith('.png')) {
            embedded = await pdfDoc.embedPng(imageBytes as any);
          } else {
            // default to JPG
            embedded = await pdfDoc.embedJpg(imageBytes as any);
          }

          if (embedded) {
            const imgWidth = Math.min(colWidth - 20, 160);
            const imgHeight = (imgWidth * 40) / Math.max(1, imgWidth); // simple fixed height fallback
            const imgX = 30 + colWidth;
            const imgY = y + 5;
            page.drawImage(embedded, { x: imgX, y: imgY, width: imgWidth, height: 40 });
          }
        }
      } catch (e) {
        // ignore image embedding failures — do not block report generation
      }
    }

    this.drawText(pdfDoc, page, input.signatoryTitle ? input.signatoryTitle : "Principal", 30 + colWidth, y + 15, this.FONT_SIZE_SMALL, true);
    page.drawLine({
    ...
    const nameToShow = input.signatoryName ?? input.principalName ?? 'Principal';
    this.drawText(pdfDoc, page, nameToShow, 30 + colWidth, y - 5, this.FONT_SIZE_SMALL, false);

    if (input.signatoryComment) {
      // render comment smaller below the name
      this.drawText(pdfDoc, page, input.signatoryComment, 30 + colWidth, y - 20, this.FONT_SIZE_SMALL - 2, false, rgb(0.4, 0.4, 0.4));
    }
